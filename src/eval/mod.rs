...
fn do_best_node_search(board: &mut board::Board, heuristic: &properties::Heuristic) -> (Option<u8>, u64) {
    ...
    let mut alpha = get_score_heuristic(board, heuristic) - 10.;
    let mut beta = get_score_heuristic(board, heuristic) + 10.;
    ...
    while !((beta - alpha < 1.) || (better_count == 1)) {
        ...
        for m in &moves {
            let undo = board.make_move(*m);
            let (mut score, leaves, hits) = search::negamax_tpt(board, heuristic, &mut hash_table, -guess, -(guess - 0.1), depth);
            board.undo_move(undo, *m);
